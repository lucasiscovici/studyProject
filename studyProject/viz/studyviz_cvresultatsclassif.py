...
class Study_CvResultatsClassif_Viz(Viz):
    ...
    def plot_ObsConfused(self,classes,preds,title=None,lim=10,limByPlots=100,elemsByRows=10,nbCols=2,**plotConfMat_kwargs):
        ...
        classes=classes if isArr else [classes]
        preds=preds if isArr else [preds]
        ...
        if len(classes) > 1:
            ...
            fig.suptitle("Observations Confused for {}".format('' if obj.name is None else obj.name), fontsize=16)
            axes=np.ravel(ax)
            # print(preds)
            for i_,(i,j,ax) in enumerate(zip(classes,preds,axes)):
                ...
                plotDigits(dm,lim=limByPlots,elemsByRows=elemsByRows,reshape=T,
                            title=title,noImg=True if i_+1 < len(classes) else False,beforeImgAx=lambda ax, c1=c1,p1=p1:ax.set_title('Class {} Predict {}'.format(c1,p1),fontsize= 12) ,ax=ax,**plotConfMat_kwargs)
                ax.set_title('Class {} Predict {}'.format(c1,p1),fontsize= 12)
            # for j in range()
            return 
        ...
        # print(title)
        return plotDigits(dm,lim=limByPlots,elemsByRows=elemsByRows,reshape=T,title=title,noImg=False,**plotConfMat_kwargs)
